...
func spaceaposs1(s string) string{
	tab:=[]rune(s)
	tab1:=[]rune{}
	...
	for i:=0; i<len(tab); i++{
		...
		if i< len(tab)-1 && i>0 && tab[i]==39 && tab[i+1]!=' ' && tab[i-1]==' '{
			tab1=append(tab1, tab[i])
			tab1=append(tab1, ' ')
		}else{
			tab1=append(tab1, tab[i])
...
func sup(t2 []string) []string{
	...
	for i:=0; i<len(t2); i++{
		if t2[i] == "(cap)" || t2[i] == "(up)" || t2[i] == "(low)" || t2[i] == "(hex)" || t2[i] == "(bin)" {
			i+=1
		}else if t2[i] == "(cap," && isNumb(t2[i+1]) || t2[i] == "(up," && isNumb(t2[i+1])  || t2[i] == "(low," && isNumb(t2[i+1]) || t2[i] == "(hex," && isNumb(t2[i+1]) || t2[i] == "(bin," && isNumb(t2[i+1])  {
			i+=2
			continue
		} else{
			t=append(t, t2[i])
...
func IsAposs(s string) bool{
	t:=[]rune(s)
	if t[0]==39 || t[len(t)-1]==39 {
		return true
	}
	return false
}
...
//	conv:=double(conv1)
	...
	for i:=0; i<len(tabString); i++{
		if instruction1(tabString[0]){
			tabString=supprime2(tabString, 0)
			i-=1
		}
		if instruction2(tabString[0]) && isNumb(tabString[1]){
			tabString=supprime3(tabString, 0)
			i-=2
		}
		...
		if i>0 && tabString[i]=="(cap," && isNumb(tabString[i+1]){
		...
		if i > 0 && tabString[i]=="(up," && isNumb(tabString[i+1]){
			limit:=trier(tabString[i+1])
			for j:=i-1; j>i-limit-1; j--{
				if j<0{
					break
		...
		if i > 0 && tabString[i]=="(low," && isNumb(tabString[i+1]){
			limit:=trier(tabString[i+1])
			for j:=i-1; j>i-limit-1; j--{
				if j<0{
					break
		...
		if i > 0 && tabString[i]=="(hex," && isNumb(tabString[i+1]){
			limit:=trier(tabString[i+1])
			for j:=i-1; j>i-limit-1; j--{
				if j<0{
					break
		...
		if i > 0 && tabString[i]=="(bin," && isNumb(tabString[i+1]){
			limit:=trier(tabString[i+1])
			for j:=i-1; j>i-limit-1; j--{
				if j<0{
					break
